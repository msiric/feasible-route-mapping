import { MAX_LOCATIONS } from './requests.mjs';
...
export function DemoBar() {
 ...
 async function showSample() {
  usePreviousCalculation.getState().resetPreviousCalculation();
  setLive(false);useShortestPath.getState().resetShortestPath();useIsochroneIntersections.getState().resetIsochroneIntersections();
  try {
   const response=await fetch('/demo/sample.json');if(!response.ok)throw new Error();const sample=await response.json();if(useDemo.getState().live)return;
   ...
   setMessage('Coast to city · 4 stops · drive, walk and cycle.');
 ...
 return <Box sx={{mb:2}}>
  ...
  {!live && <Typography variant="body2" sx={{mb:1}}>Glenelg → Central Market → Art Gallery → Norwood. Colored bands show how extra time opens up alternative paths on each leg. This real, precomputed example loads without the routing server.</Typography>}
  ...
  <details style={{fontSize:12,marginTop:8}}><summary>Scope, accuracy and privacy</summary>
   <p>South Australia only. Up to {MAX_LOCATIONS} locations, eight road exclusions, and 40 minutes per segment including zero to ten extra minutes. Bus means road routing for a bus, without public transport timetables.</p>
   <p>The colored areas are an approximation from one-minute time slices and generalized road-network contours. They can miss narrow areas and be truncated at the regional boundary. The reference route uses mode-specific preferences; it is not a guaranteed minimum-time route. Static regional data can omit implicit country or time-dependent road rules. No live traffic or turn-by-turn safety guidance.</p>
