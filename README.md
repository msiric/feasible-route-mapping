...
**Restoration status:** frontend and gateway tests pass. The real Docker engine and bundled example are being validated before the public live demo is announced. Hosting uses isolated `msiric-public-demos` projects on Cloudflare Pages Free and Render Free; no paid infrastructure is required. See [deployment details](DEPLOYMENT.md).
